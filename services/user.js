    ...
    if (result.length > 0) {
      ...
      bcrypt.compare(password, user.password, (error, isValid) => {
        if (isValid) {
          ...
          let accessToken = jwt.sign(payload, SECRET_ACCESS_TOKEN, {
            expiresIn: "15m",
          });
          let refreshToken = jwt.sign(payload, SECRET_REFRESH_TOKEN, {
            expiresIn: "30m",
          });
          let data = {
...
export const tokenValidation = (request, response, next) => {
  ...
  if (accessToken) {
    ...
    errorResponse(
      response,
      "invalid request, authorization header not found!!"
    );
  }
};

export const newTask = async (request, response, next) => {
  try {
    let task_id = request.body.task_id;
    let task_name = request.body.task_name;
    let task_description = request.body.task_description;
    let is_done = request.body.is_done;
    bcrypt.hash(password, async (error, hashedPass) => {
      ...
      if (result.insertId > 0) {
        successResponse(response, "success create task", result.insertId);
      } else {
        errorResponse(response, "failed create user", 500);
      }
    });
...
export const getTaskDetail = async (request, response, next) => {
  try {
    let id = request.params.id;
    let [result] = await getIdTask(id);
